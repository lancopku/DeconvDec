...
class luong_attention(nn.Module):
    ...
    def __init__(self, hidden_size, emb_size, pool_size=0):
        ...
        if pool_size > 0:
            ...
            self.linear_out = nn.Sequential(nn.Linear(2*hidden_size, hidden_size), nn.SELU(), nn.Linear(hidden_size, hidden_size), nn.SELU(), nn.Dropout(p=0.4))
...
class luong_gate_attention(nn.Module):
    ...
    def __init__(self, hidden_size, emb_size, prob=0.3, selfatt=False):
        ...
        self.linear_in_conv = nn.Sequential(nn.Linear(hidden_size, hidden_size), nn.Dropout(p=prob))
        self.linear_out = nn.Sequential(nn.Linear(2*hidden_size, hidden_size), nn.SELU(), nn.Dropout(p=prob))
        self.linear_out_conv = nn.Sequential(nn.Linear(3*hidden_size, hidden_size), nn.SELU(), nn.Dropout(p=prob))
        self.softmax = nn.Softmax(dim=1)
    ...
    def forward(self, h, convs):
        if self.selfatt:
            gamma_enc = convs # Batch_size * Length_enc * Hidden_size
            gamma_h = h # Batch_size * Hidden_size * Length
            weights = torch.bmm(gamma_enc, gamma_h) # Batch_size * Length_enc * Length
            weights = self.softmax(weights/math.sqrt(512))
            c_t = torch.bmm(weights.transpose(1,2), gamma_enc) # Batch_size * Length * Hidden_size
            output = self.linear_out(torch.cat([gamma_h.transpose(1,2), c_t], 2)) # Batch_size * Length * Hidden_size
            return output
        else:
            x = h
            ...
            gamma_c = self.linear_in_conv(x).unsqueeze(2)
            weights_conv = torch.bmm(convs, gamma_c).squeeze(2)
            weights_conv = self.softmax(weights_conv)
            c_conv = torch.bmm(weights_conv.unsqueeze(1), convs).squeeze(1)
            output = self.linear_out_conv(torch.cat([c_t, h, c_conv], 1))

            return output, weights, weights_conv
